        ...
        <div class="uk-container">
          ...
          <div class="uk-card uk-card-default tm-ignore-container">
              <div class="uk-grid-collapse uk-child-width-1-3 uk-child-width-1-4@m tm-products-grid" uk-grid>
                  <article v-for="product in fuzzy(products,search)" :key="product.id" class="tm-product-card">
          ...
          <div class="uk-margin uk-text-center">
              <router-link class="uk-link-mute uk-text-uppercase tm-link-to-all" to="/all-products">
                <span>shop all</span>
...
export default {
  ...
  data() {
    return {
      order: '',
      // ref2: fb.firestore().collection('users'),
      search: '',
      key: '',
      label: 'new',
      product: {},
      products: [],
      ref: fb.firestore().collection('products').orderBy('timestamp','desc'),
    }
  },
  ...
  created () { 
    ...
    // this.ref2.doc(user.uid).collection('orders').get().then(res => {
  ...
  computed: {
    ...mapGetters(["maxi"]),
    finalAccount() {
      return parseFloat(this.order) + parseFloat(this.$store.getters.totalFavorite)
    }
  },
  methods: {
    getImage(images) {
